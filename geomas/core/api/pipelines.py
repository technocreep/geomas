...
class PipelineApi:
    ...
    def chroma_loading(self, path: str, collection: str) -> None:
    ...
    def chroma_view(self, query: str, collection: str,
                    k: int = 1, embedding_function: HuggingFaceHubEmbeddings = None) -> list:
    ...
    def proto_view(self, query: str, collection: str, k: int = 1) -> list:
        # Returns k chunks that are closest to the query
        embedding_function = HuggingFaceHubEmbeddings(model=default_settings.embedding_host)
        chroma_client = init_chroma_client()
