...
class VisualDataProcessor:
    ...
    def process_visual_documents(
        ...
        for img_path in image_files:
            ...
            try:
                if skip_existing and json_path.exists():
                    ...
                    doc_metadata = doc_record.get("metadata", {})
                    description = doc_record.get("page_content", {})
                    doc = Document(page_content=description,
                                    metadata=doc_metadata)
                    documents.append(doc)
                    logger.info(f"Loaded existing document: {img_path.name}")
                    continue

                logger.info(f"Processing new image: {img_path.name}")
        ...
        logger.info(f"Loaded/Processed {len(documents)} total documents")
        return documents
