...
class VLMProcessor:
    ...
    def _init_model(self):
        ...
        if self._llm_model is None:
            try:
                ...
                elif 'http' in model_url:
                # Lazy import to avoid importing unsloth/triton at module load time
                    from geomas.core.inference.api_inference import APIConnector
                    self._llm_model=APIConnector(model_name="qwen3-vl:2b",base_url=self.model_url)
    ...
    def describe_image(
        ...
        try:
            ...
            result = self._llm_model.invoke(messages, inference_config=self.inference_params, image_path=image_path)[0]
            
            description = result.get("response", "")[0] if isinstance(result, dict) else str(result)
